...
def classify_image(image_base64_data, file_path=None):
    ...
    result = []
    for img in imgs:                                                # same function the we used for model building
        scalled_raw_img = cv2.resize(img, (32, 32))
        img_har = w2d(img, 'db1', 5)
        scalled_img_har = cv2.resize(img_har, (32, 32))
        combined_img = np.vstack((scalled_raw_img.reshape(32 * 32 * 3, 1), scalled_img_har.reshape(32 * 32, 1)))
                                                         # r, g, b image                      # gray image only 2D
        len_image_array = 32*32*3 + 32*32

        final = combined_img.reshape(1,len_image_array).astype(float)

        result.append({
            'class': class_number_to_name(__model.predict(final)[0]),      # prediction
...
def load_saved_artifacts():
    ...
    global __model
    if __model is None:                                                     # saved model is used here
        with open('./artifacts/saved_model.pkl', 'rb') as f:
            __model = joblib.load(f)
    print("loading saved artifacts...done")


def get_cv2_image_from_base64_string(b64str):           # it uses string and returns image
    '''
...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):        # same function the we used for model building
